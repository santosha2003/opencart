	...
	<div class="panel panel-default <%= parseInt(model.config.display) ? '' : 'hidden' %>">
		...
		<div class="qc-checkout-product panel-body" >
			...
			<% if(model.error){ %>
				...
				<% } %>
			<% } %>
			<div class="table-responsive" style="padding-left: 20px; border: none; margin-left: -20px;">
			<table class="table-responsive-xs table cart-table qc-cart">
				<thead>
					<tr>
						<th class="qc-image <%= parseInt(model.config.columns.image) ? '' : 'hidden' %>">Товар</th>
						<th class="qc-name <%= parseInt(model.config.columns.name) ? '' : 'hidden' %>"></th>
						<th class="qc-model hidden-xs <%= parseInt(model.config.columns.model) ? '' : 'hidden' %>"><?php echo $column_model; ?>:</th>
						<th class="qc-price hidden-xs <%= parseInt(model.config.columns.price) && model.show_price ? '' : 'hidden' %>">Цена</th>
						<th class="qc-quantity <%= parseInt(model.config.columns.quantity) ? '' : 'hidden' %>"><?php echo $column_quantity; ?></th>
						<th class="qc-total <%= parseInt(model.config.columns.total) && model.show_price ? '' : 'hidden' %>">Сумма</th>
						<th class="btn-dng"></th>
					</tr>
				</thead>
				...
				<tbody>
					...
					<tr <%= product.stock ? '' : 'class="stock"' %>>
						...
						<td class="qc-quantity <%= parseInt(model.config.columns.quantity) ? '' : 'hidden' %>">
							<div class="input-group input-group-sm">
								...
								<!--<span class="input-group-btn">-->
									<button class="btn-down decrease hidden-xs" data-product="<%= product.key %>"><img class="img-btn" src="catalog/view/theme/SnowWhite/img/minus.png"></button>
								<!--</span> -->           
								...
								<!--<span class="input-group-btn">-->
									<button class="btn-up increase hidden-xs" data-product="<%= product.key %>"><img class="img-btn" src="catalog/view/theme/SnowWhite/img/plus.png"></button>
						...
						<td class="btn-dang">
							<button class="delete hidden-xs" onclick="cart.remove('<%= product.key %>');"><i class="fa fa-times"></i></button>
						</td>
					</tr>
					<% }) %>
			...
			<div class="form-horizontal d-flex flex-row-reverse coupon-section row" style="padding-right:15px;">
				<div class="col-md-3 qc-coupon <%= parseInt(model.config.option.coupon.display) ? '' : 'hidden' %>">
					...
					<% } %>
					<label class="col-md-4 col-sm-12 control-label coupon-section-label" >
						КУПОН
					</label>
				...
				<div style="padding-left: 45px;" class="col-md-4 qc-voucher <%= parseInt(model.config.option.voucher.display) ? '' : 'hidden' %>">
					...
					<div class="col-md-8 col-sm-12" style="padding-left: 35px;">
						<div class="input-group">
			...
			<div class="form-horizontal qc-totals">
				...
				<div class="row">
					...
					<div class="form-control-static text-right total-bottom"><%= total.title %>: <span id="total-total-total"><%= total.text %></span></div>
				</div>
				<% }) %>
			</div>
			<% } %>
