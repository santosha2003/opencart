	...
	<div class="login-btn-group">
		...
		<% if(parseInt(model.config.option.register.display)){ %>
			<div class="btn-group" role="group" style="width: 100%;">
				<input  class="hidden btn_toogler" type="radio" name="account" id="register" value="register" autocomplete="off" <%= model.account == 'register' ? 'checked="checked"' : '' %> >
				<label for="register" class="label_toogler <%= model.account == 'register' ? '' : '' %> <%= parseInt(model.config.option.register.display) ? '' : 'hidden' %>">
