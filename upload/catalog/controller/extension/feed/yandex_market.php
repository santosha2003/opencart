...
//				$data['cbid'] = 15;
				...
				if ($product['price']>0) {
					...
					$min_price = $this->model_export_yandex_market->getProductOptionMinPrice($product['product_id']);
				}
				$data['price'] = number_format($this->currency->convert($this->tax->calculate($min_price, $product['tax_class_id']), $shop_currency, $offers_currency), $decimal_place, '.', '');
...
//		}
		...
		foreach ($allowed_tags as $key => $value) {
			if ($key == 'description' && $html_description) {
