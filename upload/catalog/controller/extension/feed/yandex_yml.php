 ...
 */
//Версия модуля
define ('YANDEX_YML_VERSION', '1.9.3');
...
//---- Config section ----
	...
	*/
	...
	protected function getYmlChunk($page = 0) {
		...
		foreach ($this->model_extension_feed_yandex_yml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers, $this->customer_group, $product_rel || $product_accessory, $skip, $limit, $this->is_main_category) as $product) {
			$is_out = false;
			$data = array();
...
//				$data['cbid'] = 15;
			...
			if ($this->config->get($this->CONFIG_PREFIX.'shop_sku_field')) {
				$data['shop-sku'] = $product[$this->config->get($this->CONFIG_PREFIX.'shop_sku_field')];
                if ($data['shop-sku'] == '') {
                    unset($data['shop-sku']);
                }
			}
			...
			if ($this->config->get($this->CONFIG_PREFIX.'quantity_in_stock')) {
				$data['quantity_in_stock'] = $product['quantity'];
			}
		...
		if (count($coptions)) {
			foreach ($coptions as $option) {
				...
				if ($this->config->get($this->CONFIG_PREFIX.'quantity_in_stock') && $option['subtract']) {
					$data_arr['quantity_in_stock'] = $option['quantity'];
				}
	...
	*/
	...
	protected function setOffer($data) {
		...
		$attributes = array('id', 'type', 'available', 'bid', 'cbid', 'param', 'delivery-option', 'group_id', 'accessory', 'age', 'condition', 'opt_prices', 'selling_type');
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'oldprice'=>0, 'price_old'=>0, 'purchase_price'=>0, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'picture'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0, 'min-quantity'=>0, 'step-quantity'=>0, 'minimum_order_quantity﻿'=>0, 'stock_quantity'=>0, 'quantity_in_stock'=>0);
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'description'=>0, 'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'weight'=>0, 'downloadable'=>0, 'adult'=>0, /*'age'=>0,*/ 'barcode'=>0, 'keywords'=>0, 'shop-sku'=>0, 'market-sku'=>0, 'rec'=>0));
	...
	protected function getYmlFooter() {
        ...
        if ($this->config->get($this->CONFIG_PREFIX.'gift_promo_gift') != '') {
            ...
            foreach($promo_gift as $gift_id=>$gift_name) {
                ...
                $product_ids = array_intersect($this->product_ids, $this->model_extension_feed_yandex_yml->getFilteredProducts($promo_field[$gift_id], $promo_val[$gift_id]));
                if (count($product_ids)) {
                    $ret.= '<promo id="gift'.$gift_id.'" type="gift with purchase">'
		...
		if ($this->config->get($this->CONFIG_PREFIX.'flash_discount')) {
			$specials = $this->model_extension_feed_yandex_yml->getProductSpecials($this->customer_group);
			if (count($specials) > 0) {
		...
		if ($this->config->get($this->CONFIG_PREFIX.'coupons')) {
			...
			$coupons = $this->model_extension_feed_yandex_yml->getCoupons(explode(',', $this->config->get($this->CONFIG_PREFIX.'coupons')));
			if (count($coupons) > 0) {
				foreach ($coupons as $key=>$coupon) {
					$ret.= '<promo id="code'.$coupon['coupon_id'].'" type="promo code">'. $this->eol
						...
						.($coupon['date_end'] != '0000-00-00' ? '<end-date>'.$coupon['date_end'].'</end-date>' : '')
                        .'<description>'.$this->prepareField($coupon['name']).'</description>'
